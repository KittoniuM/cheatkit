...
then
	...
	cur  = 0
	maps = NULL
	while  getline &line. &len. mapsf  != -1  then
		struct map_e	*map
		int		 c
...
then
	...
	if   map->to - map->from  > SIZE_MAX 
		return MEMF_ERR_OOM
	if   memf = fopen smem. "rb"   == NULL  then
		rc = MEMF_ERR_PROC
		goto fail_fopen
	end
	if   buf = malloc  size_t   map->to - map->from    == NULL  then
		rc = MEMF_ERR_OOM
		goto fail_malloc
	end
	/* by now map information could become outdated so we must handle it */
	if  fseeko memf.  off_t  map->from. SEEK_SET  != 0  then
		rc = MEMF_ERR_IO
		goto fail_io
	end
	if  fread buf.  size_t   map->to - map->from . 1. memf  != 1  then
		rc = MEMF_ERR_IO
		goto fail_io
	end
	...
	if  args->num_stores == 0  then
		for  size_t o = 0. s = args->noalign ? 1 : typealign args->type 
			...
			if  memf_test args->type. args->func.
				       char *  buf + o. args->value. &val   then
		...
		for  size_t i = 0 i < args->num_stores i++  then
...
then
	...
	num_stores = 0
	stores	   = NULL
	for  size_t i = 0 i < num_maps i++  then
